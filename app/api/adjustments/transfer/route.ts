...
export async function POST(request: Request) {
  try {
    ...
    const { transferStockQty, warehouseId,warehouse,notes, referenceNumber }  = await request.json();
    ...
    if (!transferStockQty || !warehouseId || !notes || !referenceNumber || warehouse) {
    ...
    return NextResponse.json(transferStock, { status: 201 }); // 201 Created
